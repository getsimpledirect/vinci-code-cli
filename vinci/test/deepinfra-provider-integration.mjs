...
function registrations(qualification) {
  ...
  // The vllm lane registers on CREDENTIAL PRESENCE rather than an opt-in flag (see
  // vinci/test/vllm-provider-integration.mjs for why), so it appears here on any machine that
  ...
  const priorTelusKey = process.env.VLLM_API_KEY;
  const priorTelusUrl = process.env.VLLM_BASE_URL;
  ...
  try {
    ...
    if (priorTelusKey === undefined) delete process.env.VLLM_API_KEY;
    else process.env.VLLM_API_KEY = priorTelusKey;
    if (priorTelusUrl === undefined) delete process.env.VLLM_BASE_URL;
    else process.env.VLLM_BASE_URL = priorTelusUrl;
  }
  return seen;
}
