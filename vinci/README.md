...
- **Session transcripts are stored unmasked on disk.** Redaction is applied at display and
  egress, not at rest. Treat `~/.pi/agent/sessions/` as sensitive.
...
> 🔴 **Redaction is applied at display and egress, NOT at rest.** Session transcripts under
> `~/.pi/agent/sessions/` may contain an unmasked credential you typed or a file you read. An
> earlier version of this section claimed credentials were "redacted before session persistence",
> which was wrong in the reassuring direction and contradicted [`SECURITY.md`](../SECURITY.md) and
> [`PRIVACY.md`](PRIVACY.md). Treat that directory as sensitive.
