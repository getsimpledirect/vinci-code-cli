...
describe("openai-completions tool_choice", () => {
	...
	it("omits tool_stream for z.ai models without tool_stream compat", async () => {
		const baseModel = getModel("zai", "glm-4.7")!;
		const { zaiToolStream: _ignored, ...compatWithoutToolStream } = baseModel.compat ?? {};
		const model = { ...baseModel, compat: compatWithoutToolStream };
		const tools: Tool[] = [
			{
				name: "ping",
				description: "Ping tool",
