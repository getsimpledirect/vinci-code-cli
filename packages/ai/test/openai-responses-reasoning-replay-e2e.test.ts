...
import { complete, getEnvApiKey } from "../src/compat.ts";
...
describe.skipIf(!process.env.OPENAI_API_KEY || !process.env.ANTHROPIC_API_KEY)(
	...
	() => {
		...
		it("handles same-provider different-model handoff with tool calls", { retry: 2 }, async () => {
			...
			// 2. User switches to another reasoning-capable OpenAI Responses model
			...
			const modelA = findModelByCapability("openai", {
				api: "openai-responses",
				reasoning: true,
			});
			const modelB = findModelByCapability("openai", {
				api: "openai-responses",
				reasoning: true,
				excludeModelIds: [modelA.id],
			});
		...
		it("handles cross-provider handoff from Anthropic to OpenAI Responses", { retry: 2 }, async () => {
			...
			// 2. User switches to a reasoning-capable OpenAI Responses model
			// 3. transform-messages: isSameModel=false, thinking converted to text
			...
			const anthropicModel = findModelByCapability("anthropic", {
				api: "anthropic-messages",
				reasoning: true,
			});
			const openaiModel = findModelByCapability("openai", {
				api: "openai-responses",
				reasoning: true,
			});
			...
			// Now continue with OpenAI Responses (different provider)
			const context: Context = {
				systemPrompt: "You are a helpful assistant. Answer concisely.",
				messages: [userMessage, assistantResponse, toolResult, followUp],
				tools: [testTool],
